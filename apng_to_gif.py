...
def save_as_gif(animation: Animation, output_path: Path, colors: int = 256) -> int:
    processed_frames = []
    ...
    for i, frame in enumerate(processed_frames):
        ...
        for j, alpha_val in enumerate(a_data):
            if alpha_val < 10:  # Only fully transparent, not semi-transparent outlines
                p_data[j] = colors - 1
...
def compress_to_target_size(animation: Animation, output_path: Path, target_size_bytes: int) -> Path:
    skip_factor = 1
    colors = 256
    ...
    while skip_factor <= len(animation.frames) and colors >= 64:
        ...
        size = save_as_gif(compressed_anim, temp_path, colors)
...
def main():
    ...
    result_path = compress_to_target_size(animation, output_path, target_size_bytes)
